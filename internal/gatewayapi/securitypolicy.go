		...
		for _, currTarget := range targetRefs {
			if currTarget.Kind != resource.KindGateway && currTarget.SectionName == nil {
				...
				if currTarget.Kind == resource.KindTCPRoute {
					t.processSecurityPolicyForTCPRoute(resources, xdsIR, routeMap, policy, currTarget)
				} else {
					t.processSecurityPolicyForHTTPRoute(resources, xdsIR, routeMap, gatewayRouteMap, policy, currTarget)
				}
			}
		}
	}
...
func (t *Translator) processSecurityPolicyForHTTPRoute(
	...
	targetedRoute, resolveErr = resolveSecurityPolicyHTTPRouteTargetRef(policy, currTarget, routeMap)
...
func (t *Translator) processSecurityPolicyForTCPRoute(
	resources *resource.Resources,
	xdsIR resource.XdsIRMap,
	routeMap map[policyTargetRouteKey]*policyRouteTargetContext,
	policy *egv1a1.SecurityPolicy,
	...
	// Translate the security policy for the TCP route
	if err := t.translateSecurityPolicyForRoute(policy, targetedRoute, currTarget, resources, xdsIR); err != nil {
		status.SetTranslationErrorForPolicyAncestors(&policy.Status,
			parentGateways,
			t.GatewayControllerName,
...
func validateSecurityPolicyForTCP(p *egv1a1.SecurityPolicy) error {
	...
	for i, rule := range p.Spec.Authorization.Rules {
		...
		// Operation is not supported for TCP routes (no HTTP methods/paths)
		if rule.Operation != nil {
			return fmt.Errorf("rule %d: Operation is not supported for TCP routes", i)
		}
	}
	return nil
}
...
func resolveSecurityPolicyHTTPRouteTargetRef(
	policy *egv1a1.SecurityPolicy,
	target gwapiv1a2.LocalPolicyTargetReferenceWithSectionName,
...
// Add TCP-specific route resolver
func resolveSecurityPolicyTCPRouteTargetRef(
	policy *egv1a1.SecurityPolicy,
	target gwapiv1a2.LocalPolicyTargetReferenceWithSectionName,
	routes map[policyTargetRouteKey]*policyRouteTargetContext,
) (RouteContext, *status.PolicyResolveError) {
	// Check if the TCP route exists
	key := policyTargetRouteKey{
		Kind:      string(target.Kind),
		Name:      string(target.Name),
		Namespace: policy.Namespace,
	...
	// TCP routes don't support sectionName
	if target.SectionName != nil {
		message := fmt.Sprintf("SectionName is not supported for TCPRoute %s/%s",
			policy.Namespace, string(target.Name))
		return route.RouteContext, &status.PolicyResolveError{
			Reason:  gwapiv1a2.PolicyReasonTargetNotFound,
			Message: message,
		}
	...
	// Check for conflicts
	if route.attached {
		message := fmt.Sprintf("Unable to target TCPRoute %s, another SecurityPolicy has already attached to it",
			string(target.Name))
		return route.RouteContext, &status.PolicyResolveError{
			Reason:  gwapiv1a2.PolicyReasonConflicted,
			Message: message,
...
func (t *Translator) translateSecurityPolicyForRoute(
	...
	// NEW: Add TCP-specific validation at the beginning
	if getRouteProtocol(route) == ir.TCP {
		if err := validateSecurityPolicyForTCP(policy); err != nil {
			return fmt.Errorf("invalid SecurityPolicy for TCP route %s/%s: %w",
				route.GetNamespace(), route.GetName(), err)
		}
	}
	...
	// NEW: Skip HTTP-only features for TCP routes
	if getRouteProtocol(route) != ir.TCP {
	...
	// Authorization is supported for both HTTP and TCP routes
...
func shouldUseFilterChainMatchers(auth *ir.Authorization) bool {
	// Use filter chain matchers if we have IP-based authorization rules
