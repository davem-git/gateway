...
>>>>>>> a107a0388 (feat: support listener name in SecurityPolicy target (#5916))
...
// +kubebuilder:validation:XValidation:rule="(has(self.targetRef) && self.targetRef.kind == 'TCPRoute' || has(self.targetRefs) && self.targetRefs.exists(ref, ref.kind == 'TCPRoute')) ? (has(self.authorization) && !has(self.apiKeyAuth) && !has(self.cors) && !has(self.basicAuth) && !has(self.jwt) && !has(self.oidc) && !has(self.extAuth)) : true", message="When targeting TCPRoute, only the authorization field is supported"
//
